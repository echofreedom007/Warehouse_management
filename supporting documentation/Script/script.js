...
function sendRequest(url, method, body, callback, role) {
  ...
  xhr.onload = function () {
    ...
    if (xhr.status === 200 || xhr.status === 201 || xhr.status === 204) {
      ...
      span.innerHTML =
        "Username and/or password doesn't match, plase try again.";
      span.style.borderRadius = "5px";
      ...
      if (document.querySelector("h2") != null) {
        ...
        header2.innerHTML =
          "This username has been used. Try a new username instead.";
...
function addRow() {
  if (document.querySelector("table") != null) {
    ...
    let numberOfCol =
      document.getElementsByTagName("table")[0].rows[0].cells.length - 1;
